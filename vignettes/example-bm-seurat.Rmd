...
Running enrichment of cell type signatures across all genes avoids the problem of selecting a specific number of cluster markers.

Calculate the average expression levels for each cell type, convert to a matrix, and log-transform.
...
```{r}
enrich_tbl %>% filter(cluster == "B-cell") %>% head(10)
```

Check the most enriched cell types for the Adipo-CAR cluster.

```{r}
enrich_tbl %>% filter(cluster == "Adipo-CAR") %>% head(10)
```

Check the most enriched cell types for the Osteoblasts cluster.

```{r}
enrich_tbl %>% filter(cluster == "Osteoblasts") %>% head(10)
```
